...
var presntRow = function presntRow(row) {
    var tr = $('<tr id="tr_' + row.id + '"></tr>');
    var columns = ['id', 'title', 'body', 'userId'].map(function (field) {
        return $('<td><pre>' + row[field] + '</pre></td>');
    });
    var manipulationUrl = baseUrl + '/' + row.id;
    ...
    _lodash2.default.each(columns, function (c) {
        return tr.append(c);
    });
    var buttonsTd = $('<td></td>');
...
function getData() {
    $.ajax(baseUrl, {
        ...
        _lodash2.default.each(data, function (item) {
            return $('#microPosts').append(presntRow(item));
        });
    });
}
